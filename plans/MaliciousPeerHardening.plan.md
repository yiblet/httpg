...
| # | Case | Direction / proto | Go mechanism (ref) | httpg today | Verdict | Ticket |
|---|------|-------------------|--------------------|--------------|---------|--------|
...
- **Per-Ticket Workflow (each sub-agent):**
  ...
  4. Add the named ported/adapted test(s) into the `Abuse` suite (or the existing suite the test belongs to) and wire into `test/test_httpg.ml`.
...
- `test/test_abuse.ml` (new) + wired `("Abuse", Test_abuse.tests)` into `test/test_httpg.ml`: `slowloris_header_timeout` and `idle_timeout`, both raw-loopback-socket integration tests bounded by `Net.with_timeout`.
...
- `test/test_abuse.ml`: added `expect_100_continue` and `expect_unknown` (raw-loopback integration, bounded by `Net.with_timeout`), plus a `read_all_until_eof` helper. Wired both into the `tests` list. (No change to `test/test_httpg.ml` — the `Abuse` suite was already aggregated by Ticket 1.)
...
- `test/test_abuse.ml`: added `response_header_too_large` and `response_header_under_limit_ok`, both driving a RAW `Net.listen`/`Net.accept` loopback server (not a httpg `Server`, so it can emit malicious bytes) against a `Client` backed by `Transport.create ~max_response_header_bytes:8192`, all bounded by `Net.with_timeout 5.`. Added a `with_raw_server` helper (single-shot accept + serve, listener/conn torn down in `finalize`). The too-large test asserts the round trip fails with an exception whose message contains `MaxResponseHeaderBytes` (not a hang/OOM); the ok test asserts 200 + intact body.
...
Full test results in `~/workspace/httpg/_build/default/test/_build/_tests/httpg'.
...
- **Chosen values:** list size (and thus per-string cap) default = `1 lsl 20` (1 MiB), matching Go's `DefaultMaxHeaderBytes`. This is the safe, faithful httpg default; it is configurable per-server via `Server.create ~max_header_bytes` / `H2_server.serve ~max_header_bytes`.
...
- The actual malicious-peer flow-control-overflow `ConnectionError(ErrCodeFlowControl)` lives on **two** paths, both of which httpg **already** models correctly: (1) inbound DATA over the window — `inflow.take`/`takeInflows` false → `streamError`/`ConnectionError` (server.go:1444,:1732,:1749,:1762) ⇒ httpg `process_data` already returns `Error H2_error.FlowControlError` (h2_server.ml:857-915); (2) a bogus peer **WINDOW_UPDATE** overflowing the *outbound* window — `flow.add` false → `ConnectionError(ErrCodeFlowControl)` (server.go:1525-1529,:1686-1693) ⇒ httpg `process_window_update` already returns `Error H2_error.FlowControlError` (h2_server.ml:998-1007), riding `outcome_of_result`→`Conn_error`→`go_away`. Go's overflow test `TestServer_Send_GoAway_After_Bogus_WindowUpdate` (server_test.go:1354-1365) exercises exactly path (2) and already passed here.
