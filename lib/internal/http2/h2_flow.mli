...
(** [inflow_add f n] adds [n] bytes to the window, returning the number of
    bytes to send in a WINDOW_UPDATE frame (0 when buffered). Mirrors
    [inflow.add]. Raises [Invalid_argument] on negative update or when the
    window would exceed 2^31-1 (Go panics). *)
...
(** [available f] is the number of bytes that may be sent, the min of the
    stream and (if linked) conn windows. Mirrors [outflow.available]. *)
val available : outflow -> int32

(** [take f n] consumes [n] bytes from the window (and the linked conn window).
    Mirrors [outflow.take]. Raises [Invalid_argument] if [n > available f]
    (Go panics). *)
