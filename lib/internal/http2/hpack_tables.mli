...
(** A name-value pair. Both name and value are opaque octet sequences. Mirrors
    Go's [HeaderField]. *)

val is_pseudo : header_field -> bool
(** [is_pseudo hf] reports whether the field name starts with a colon. Mirrors
    Go's [HeaderField.IsPseudo]. *)
...
(** [search t f] finds [f]. Returns [(0, false)] for no match; [(i, true)] when
    name and value match; [(i, false)] when only the name matches. The returned
    index is a 1-based HPACK index. Mirrors Go's [search]. *)
...
type dynamic_table
(** The HPACK dynamic table with size accounting and eviction. Mirrors Go's
    [dynamicTable]. *)
...
(** [dynamic_table_of dt] exposes the underlying field table (for indexed lookup
    / search by the encoder). Mirrors [dynamicTable.table]. *)
