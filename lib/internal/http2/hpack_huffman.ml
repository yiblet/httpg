...
let new_internal_node () =
  { children = Array.make 256 None; code_len = 0; sym = 0 }
  ...
  String.iter
    (fun ch ->
      ...
      if !n >= 32 then begin
        ...
        Buffer.add_char dst
          (Char.chr (to_int (logand (shift_right_logical y 24) 0xFFL)));
        Buffer.add_char dst
          (Char.chr (to_int (logand (shift_right_logical y 16) 0xFFL)));
        Buffer.add_char dst
          (Char.chr (to_int (logand (shift_right_logical y 8) 0xFFL)));
