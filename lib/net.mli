...
(** [listen ?backlog host port] resolves [host]/[port], creates a TCP socket
    ...
    ["0.0.0.0"] or ["127.0.0.1"]). Binding [port = 0] selects an ephemeral port;
    recover it with {!local_addr}/{!bound_port}. [backlog] defaults to 128 (Go's
    [net.Listen] default). *)
...
(** [connect ~host ~port ?tls ?authenticator ?insecure ()] resolves
    ...
    - [?insecure:true] — use {!null_authenticator} (no verification at all), the
      analogue of Go's [InsecureSkipVerify = true]. When neither is given the
      secure {!default_authenticator} is used. Note that verifying against an IP
      literal [host] (no valid hostname) legitimately fails name matching; such
      callers must opt out via [?insecure]. *)
...
(** [test_server_certificate ()] mints a fresh self-signed RSA-2048 certificate
    + key at runtime (no files on disk), CN=localhost with SubjectAltName
      ...
      [net/http/internal/testcert]. Intended for tests/loopback servers; since
      the matching client uses {!null_authenticator}, the cert only satisfies
      the handshake's server-certificate step (no real trust). *)
...
(** [bound_port fd] is the locally bound TCP port (handy for ephemeral
    [listen]ers bound on port 0). Raises [Failure] if [fd] is not an [ADDR_INET]
    socket. *)
