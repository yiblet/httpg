...
(** [make_stream ?max_frame_size id] creates a {!stream} with a fresh
    zero-valued outflow and the given (default {!H2.initial_max_frame_size})
    max frame size. *)
...
(** [consume wr n] consumes [min(n, available)] bytes from [wr], where
    ...
    - [(consumed, rest, 2)] if it was split — [consumed] carries the leading
      ...
      stream's flow window. Non-DATA frames are always consumed whole.
    Mirrors Go's [FrameWriteRequest.Consume]. *)
val consume :
  frame_write_request ->
  int ->
  frame_write_request * frame_write_request * int

(* ---- the scheduler ---- *)

(** The round-robin write scheduler. Mirrors Go's
...
(** [open_stream ws id] opens stream [id] in the scheduler, appending its queue
    ...
    stream (raises [Failure], mirroring Go's panic). Mirrors
    [OpenStream]. *)
