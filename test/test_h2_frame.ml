...
let test_frame_type_string () =
  ...
  Alcotest.(check string)
    "GOAWAY" "GOAWAY"
    (H2.Private.frame_type_string H2.Goaway)
...
let test_write_priority () =
  ...
  let f =
    with_pipe (fun oc -> F.Private.write_priority oc 42 p) (read_one ())
  in
  (match f with
  | F.Priority (fh, pf) ->
...
let test_frame_header_codec () =
  ...
  let h2 =
    F.Private.decode_frame_header "\x01\x02\x03\x04\x05\x06\x07\x08\x09"
  in
  ...
  (* high bit masked *)
  let h3 =
    F.Private.decode_frame_header "\xff\xff\xff\xff\xff\xff\xff\xff\xff"
  in
...
let test_read_oversize_frame_result () =
  ...
  | Error e ->
      Alcotest.failf "expected Error Frame_too_large, got Error %s"
        (match e with
        | H2_error.Connection c ->
            "Connection " ^ H2_error.Private.err_code_string c
        | _ -> "<other>")
