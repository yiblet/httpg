...
let var_int_need_more () =
  (* a multi-byte varint that is truncated -> Need_more *)
  let buf = Buffer.create 4 in
  H.append_var_int buf 5 1337;
  let s = String.sub (Buffer.contents buf) 0 1 in
  match H.read_var_int 5 s 0 with
  | Error H.Need_more -> ()
  | _ -> Alcotest.fail "expected Need_more"
...
let c2_1_literal_with_indexing () =
  ...
  let got = H.decode_full dec expected in
...
let c3_request_sequence () =
  ...
  in
  let g1 = H.decode_full dec b1 in
  ...
  let g2 = H.decode_full dec b2 in
  ...
  in
  let g3 = H.decode_full dec b3 in
...
let roundtrip_basic () =
  ...
  let dec = H.new_decoder 4096 (fun _ -> ()) in
  let decoded = H.decode_full dec encoded in
...
let roundtrip_dynamic_two_passes () =
  ...
  let dec1 = H.decode_full dec enc1 in
  ...
  let dec2 = H.decode_full dec enc2 in
...
let decode_invalid_index () =
  ...
  let dec = H.new_decoder 4096 (fun _ -> ()) in
  Alcotest.check_raises "invalid indexed 0"
    (H.Invalid_indexed 0)
    (fun () -> ignore (H.decode_full dec (of_hex "80")))

let decode_index_too_large () =
  (* indexed field referencing index way past the table *)
  let dec = H.new_decoder 4096 (fun _ -> ()) in
  Alcotest.check_raises "invalid indexed large"
    (H.Invalid_indexed 200)
    (fun () -> ignore (H.decode_full dec (of_hex "ff49")))
...
let decode_truncated () =
  ...
  Alcotest.check_raises "truncated headers"
    (H.Decoding_error "truncated headers")
    (fun () -> ignore (H.decode_full dec (of_hex "400a6375")))

let decode_max_str_len () =
  let dec = H.new_decoder 4096 (fun _ -> ()) in
  ...
  in
  Alcotest.check_raises "string too long" H.String_too_long (fun () ->
      ignore (H.decode_full dec b))
...
let decode_size_update_too_large () =
  ...
  Alcotest.check_raises "size update too large"
    (H.Decoding_error "dynamic table size update too large")
    (fun () -> ignore (H.decode_full dec (of_hex "25")))
...
let decode_size_update_not_first () =
  ...
  Alcotest.check_raises "size update not at start"
    (H.Decoding_error
       "dynamic table size update MUST occur at the beginning of a header \
        block")
    (fun () -> ignore (H.decode_full dec b))
...
let tests =
  [
    ...
    ("decode_max_str_len", `Quick, decode_max_str_len);
