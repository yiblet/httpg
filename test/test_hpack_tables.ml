...
let huffman_roundtrip () =
  ...
  let cases =
    [
      ...
      "\x00\x01\x02\xfd\xfe\xff"; (* high-code symbols (long codes) *)
      String.init 256 Char.chr; (* every byte value *)
    ]
  in
  List.iter
    (fun s ->
...
let huffman_decode_invalid () =
  ...
  let bad_cases =
    [
      "\xff\xff\xff\xff"; (* all ones: incomplete symbol / overlong padding *)
      "\x00"; (* '0' code is 5 bits 00000; trailing 3 zero bits are not an
                 EOS prefix -> invalid *)
    ]
  in
  List.iter
...
let static_search_cases () =
  ...
  check_static "accept-encoding gzip,deflate"
    (hf "accept-encoding" "gzip, deflate") 16 true;
  ...
  check_static ":method GET sensitive" (hf ~sensitive:true ":method" "GET") 3
    false
...
let dynamic_table_search () =
  ...
  let entries =
    [
      hf "key1" "value1-1"; hf "key2" "value2-1"; hf "key1" "value1-2";
      hf "key3" "value3-1"; hf "key4" "value4-1"; hf "key2" "value2-2";
      hf "key3" "value3-2";
    ]
  in
...
let field_size () =
  Alcotest.(check int) "size custom-key/custom-header"
    (10 + 13 + 32)
...
let dynamic_table_add_evict () =
  ...
  T.dynamic_add dt (hf ":path" "/"); (* size 5+1+32 = 38; total 80 *)
  ...
  T.dynamic_add dt (hf ":scheme" "https"); (* 7+5+32 = 44 ; total 80+44=124 *)
...
let dynamic_set_max_size () =
  ...
  T.dynamic_add dt (hf "a" "b"); (* 1+1+32 = 34 *)
  T.dynamic_add dt (hf "c" "d"); (* 34 ; total 68 *)
