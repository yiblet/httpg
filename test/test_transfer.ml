...
let test_parse_content_length_result () =
  ...
  | other ->
      Alcotest.failf "\"42\" -> %s; want Ok 42"
        ...
        | Ok n -> Printf.sprintf "Ok %Ld" n
        | Error e -> "Error " ^ Transfer.error_to_string e));
  let h = Header.empty |> Header.set_values "Content-Length" [ "5"; "6" ] in
  match
...
let test_parse_transfer_encoding () =
  let run te =
    let h = Header.empty |> Header.set_values "Transfer-Encoding" te in
  ...
  let h = Header.empty |> Header.set_values "Transfer-Encoding" [ "chunked" ] in
  Alcotest.(check bool)
...
let test_fix_length () =
  let mk pairs =
    List.fold_left
      (fun h (k, v) -> Header.set_values k [ v ] h)
      Header.empty pairs
  in
  ...
  let hd = Header.empty |> Header.set_values "Content-Length" [ "5"; "5" ] in
  check "dup identical CL -> value" ~is_response:false ~status:200
    ~request_method:"POST" ~header:hd ~chunked:false 5L;
  let hc = Header.empty |> Header.set_values "Content-Length" [ "5"; "6" ] in
  match
    Transfer.Private.fix_length ~is_response:false ~status:200
...
let test_should_close () =
  let mk conn =
    match conn with
    | Some v -> Header.empty |> Header.set_values "Connection" [ v ]
    | None -> Header.empty
  in
...
let test_fix_trailer () =
  let mk_tr v = Header.empty |> Header.set_values "Trailer" [ v ] in
  Alcotest.(check bool)
...
let test_write_body_chunked () =
  let tw =
    Transfer.make_transfer_writer ~method_:Httpg_base.Method.Put
      ~body:(Body.of_string "hello") ~content_length:(-1L)
...
let test_write_body_fixed () =
  let tw =
    Transfer.make_transfer_writer ~method_:Httpg_base.Method.Put
      ~body:(Body.of_string "hello") ~content_length:5L ~transfer_encoding:[] ()
  in
  let out = with_output_string (fun w -> Transfer.write_body w tw) in
...
let test_write_body_length_mismatch () =
  let tw =
    Transfer.make_transfer_writer ~method_:Httpg_base.Method.Put
      ~body:(Body.of_string "hello") ~content_length:3L ~transfer_encoding:[] ()
  in
...
let test_write_body_chunked_stream () =
  ...
  let tw =
    Transfer.make_transfer_writer ~method_:Httpg_base.Method.Put
      ~body:(stream_body chunks) ~content_length:(-1L)
      ~transfer_encoding:[ "chunked" ] ()
  in
  let out = with_output_string (fun w -> Transfer.write_body w tw) in
  Alcotest.(check string)
...
let test_write_body_fixed_stream () =
  ...
  let tw =
    Transfer.make_transfer_writer ~method_:Httpg_base.Method.Put
      ~body:(stream_body chunks) ~content_length:(Int64.of_int total)
      ~transfer_encoding:[] ()
  in
  let out = with_output_string (fun w -> Transfer.write_body w tw) in
  Alcotest.(check string)
...
let test_write_body_fixed_stream_mismatch () =
  ...
  let tw =
    Transfer.make_transfer_writer ~method_:Httpg_base.Method.Put
      ~body:(stream_body chunks) ~content_length:20L ~transfer_encoding:[] ()
  in
  match with_output_string (fun w -> Transfer.write_body w tw) with
  | _ -> Alcotest.fail "expected ContentLength mismatch error"
  | exception Invalid_argument _ ->
...
let test_read_transfer_content_length () =
  let h = Header.empty |> Header.set_values "Content-Length" [ "5" ] in
  let msg =
    {
...
let test_read_transfer_bad_chunk () =
  ...
  let h_bad_te =
    Header.empty |> Header.set_values "Transfer-Encoding" [ "fugazi" ]
  in
  ...
  let h_chunked =
    Header.empty |> Header.set_values "Transfer-Encoding" [ "chunked" ]
  in
...
let test_chunked_auto_select_post () =
  ...
  let tw =
    Transfer.make_transfer_writer ~method_:Httpg_base.Method.Post ~body
      ~content_length:(-1L) ~transfer_encoding:[] ()
  in
  Alcotest.(check (list string))
